...
def enstr(key, value):
    kv = key + value
    enstr = str(base64.b64encode(kv.encode("utf-8")), 'utf-8').replace('=', '')
    node = execjs.get()
    ctx = node.compile(open('../js/md5.js', encoding='utf-8').read())
    return ctx.eval('hex_md5("{}")'.format(enstr))


def main(url, params, headers):
    response = requests.get(url=url, params=params, headers=headers).text

    json0 = json.loads(response)
    key = json0['key']
    value = json0['value']
    number = enstr(key, value)

    td_list = re.findall('<td>(.*?)</td>', json0['info'])
    n2 = []
    for td in td_list:
        img_list = re.findall('<img (.*?)>', td)
        n1 = []
        for i in range(len(img_list)):

            img_number, = re.findall('img_number (.*)\" style', img_list[i])
            if number == img_number:
                pass
            else:
                n1.append(img_list[i])

        n2.append(n1)
    return n2


def deal(url, params, headers):
    n2 = main(url, params, headers)
    n1 = 0
    for i in n2:
        dict2 = {}
        for j in range(len(i)):
            style, = re.findall('style="left:(.*)px', i[j])
            num = re.findall('base64,(.*)\" class', i[j])[0].replace('=', '')
            if (style == '0') | (style == '0.0'):
                ...
                j = j - 3
            dict2[str(j)] = dict0[num]
        # print(dict2)
        if len(dict2.values()) == 4:
            n = int('{0}{1}{2}{3}'.format(dict2['0'], dict2['1'], dict2['2'], dict2['3'], ))
            n1 = n1 + n
        elif len(dict2.values()) == 3:
            n = int('{0}{1}{2}'.format(dict2['0'], dict2['1'], dict2['2']))
            n1 = n1 + n
        elif len(dict2.values()) == 2:
            n = int('{0}{1}'.format(dict2['0'], dict2['1']))
            n1 = n1 + n
