...
WaterSecurity.prototype = {
    ...
    decode: function(r) {
        ...
        this.print(r),
        r.length < 5 && ("" == r || "null" == r)) return "[]";
        if (this.version) {
            if (r.substring(0, 3) !== this.version) return alert("加解密版本不一致！");
        ...
        for (var x = 0; 4 * x < e.length; x++) {
            var t = e.substr(4 * x, 4);
            s[x] = t,
            h[t] = null
        }
        var n = this.getTagsPosition(r, s);
        var i = 0;
    ...
    base64decode: function(r) {
        ...
        a = []; n < i;) {
            ...
            do {
                if (61 == (s = 255 & r.charCodeAt(n++))) return a.join("");
                s = this.base64DecodeChars[s]
            } while (n < i && -1 == s);
            if (-1 == s) break;
            a.push(String.fromCharCode((15 & e) << 4 | (60 & s) >> 2));
            do {
                if (61 == (h = 255 & r.charCodeAt(n++))) return a.join("");
...
resultDecode = function(e) {
        e = waterSecurity.decode(e);
        e = JSON.parse(e);
        	console.log(e)
    return e;
}
...
function getparams() {
    var rwData = {};
    rwData.time = "";
    rwData.hourClass = "1,2,3,6,12"; //,24
    rwData.name = "SelectRainWarnInfo";
    rwData.snsw = 'sn'
    var newURL = "ahsxx"
    if (true) {
        if (true) newURL = "public"
        wData = paramEncode(rwData);
    }
    return wData
}